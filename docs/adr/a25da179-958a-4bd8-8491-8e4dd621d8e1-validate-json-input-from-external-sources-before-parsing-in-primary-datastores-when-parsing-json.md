...
1. SHOULD: When parsing JSON from encrypted sources (decrypted WebSocket data, encrypted storage), validation SHOULD occur after decryption but before data is committed to cache layers or datastores

## Policy Block

- SHOULD When parsing JSON from encrypted sources (decrypted WebSocket data, encrypted storage), validation SHOULD occur after decryption but before data is committed to cache layers or datastores
