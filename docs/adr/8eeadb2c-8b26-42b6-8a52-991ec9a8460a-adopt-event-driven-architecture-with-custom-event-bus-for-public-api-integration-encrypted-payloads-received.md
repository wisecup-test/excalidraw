...
1. MUST: All encrypted payloads received via 'client-broadcast' events MUST be decrypted using decryptPayload() before processing, with type-specific handling for WS_SUBTYPES (INIT, UPDATE, MOUSE_LOCATION, USER_VISIBLE_SCENE_BOUNDS, IDLE_STATUS)

## Policy Block

- MUST All encrypted payloads received via 'client-broadcast' events MUST be decrypted using decryptPayload() before processing, with type-specific handling for WS_SUBTYPES (INIT, UPDATE, MOUSE_LOCATION, USER_VISIBLE_SCENE_BOUNDS, IDLE_STATUS)
