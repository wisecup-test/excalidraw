...
1. SHOULD: Tests SHOULD separate verification of 'once' events (which replay to late subscribers) from 'stream' events (which do not replay) using distinct test cases

## Policy Block

- SHOULD Tests SHOULD separate verification of 'once' events (which replay to late subscribers) from 'stream' events (which do not replay) using distinct test cases
