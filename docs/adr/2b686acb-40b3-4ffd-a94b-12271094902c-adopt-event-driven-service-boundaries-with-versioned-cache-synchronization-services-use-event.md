...
1. MUST: Services MUST use event-driven communication patterns (socket.on/socket.emit) for cross-boundary interactions rather than direct method invocation

## Policy Block

- MUST Services MUST use event-driven communication patterns (socket.on/socket.emit) for cross-boundary interactions rather than direct method invocation
