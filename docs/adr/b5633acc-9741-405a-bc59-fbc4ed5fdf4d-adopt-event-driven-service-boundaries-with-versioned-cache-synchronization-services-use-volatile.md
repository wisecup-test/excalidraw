...
1. SHOULD: Services SHOULD use volatile event flags (WS_EVENTS.SERVER_VOLATILE) for non-critical updates to optimize network traffic

## Policy Block

- SHOULD Services SHOULD use volatile event flags (WS_EVENTS.SERVER_VOLATILE) for non-critical updates to optimize network traffic
