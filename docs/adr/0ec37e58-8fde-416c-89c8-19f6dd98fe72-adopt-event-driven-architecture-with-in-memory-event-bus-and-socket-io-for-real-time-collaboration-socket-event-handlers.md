...
1. MUST: Socket event handlers MUST be registered using .on() pattern (e.g., this.socket.on('init-room', callback)) and properly cleaned up with .off() when no longer needed

## Policy Block

- MUST Socket event handlers MUST be registered using .on() pattern (e.g., this.socket.on('init-room', callback)) and properly cleaned up with .off() when no longer needed
