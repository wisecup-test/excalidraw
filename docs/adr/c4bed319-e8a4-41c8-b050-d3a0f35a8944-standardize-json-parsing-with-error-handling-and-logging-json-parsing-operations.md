...
1. MUST: JSON parsing operations on untrusted input (localStorage, external APIs, WebSocket data) MUST validate the parsed structure before use

## Policy Block

- MUST JSON parsing operations on untrusted input (localStorage, external APIs, WebSocket data) MUST validate the parsed structure before use
