...
1. MUST: Event bus implementations MUST cache the last payload for each event type to support replay behavior for late subscribers (e.g., this.lastPayload.get(name), this.lastPayload.set(name, args))

## Policy Block

- MUST Event bus implementations MUST cache the last payload for each event type to support replay behavior for late subscribers (e.g., this.lastPayload.get(name), this.lastPayload.set(name, args))
