...
1. MUST: Event emission MUST support both volatile and persistent modes, with socket.emit() calls distinguishing between WS_EVENTS.SERVER_VOLATILE and WS_EVENTS.SERVER

## Policy Block

- MUST Event emission MUST support both volatile and persistent modes, with socket.emit() calls distinguishing between WS_EVENTS.SERVER_VOLATILE and WS_EVENTS.SERVER
