...
1. MUST: Tests for event-driven features MUST register event listeners using .on() callbacks to capture emitted events into test-scoped arrays or collections

## Policy Block

- MUST Tests for event-driven features MUST register event listeners using .on() callbacks to capture emitted events into test-scoped arrays or collections
