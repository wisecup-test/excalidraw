...
1. SHOULD: Cache keys SHOULD use strongly-typed identifiers (e.g., element.id, fileId, socketId) rather than arbitrary strings to prevent key collisions

## Policy Block

- SHOULD Cache keys SHOULD use strongly-typed identifiers (e.g., element.id, fileId, socketId) rather than arbitrary strings to prevent key collisions
