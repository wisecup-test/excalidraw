...
- **R-IDB-001** MUST: Storage operations using idb-keyval MUST implement error handling with console.warn or console.error logging for failure scenarios.
- **R-IDB-002** MUST: All socket event handlers MUST have corresponding cleanup (socket.off) in component unmount or cleanup functions.
- **R-IDB-003** MUST: Cache layer implementations MUST use version tracking Maps for element synchronization.
- **R-IDB-004** MUST: Public API contracts (Portal, LocalData, TTDIndexedDBAdapter) MUST expose async methods with proper concurrency control.
- **R-IDB-005** SHOULD: Apply lodash.throttle to high-frequency broadcast operations (_broadcastSocketData) to prevent network flooding.
- **R-IDB-006** SHOULD: Distinguish between volatile (WS_EVENTS.SERVER_VOLATILE) and persistent (WS_EVENTS.SERVER) broadcasts based on data criticality.
...
<enforcement>
Claude Code MUST NOT skip or defer verification of these rules. All idb-keyval operations MUST include error handling. All socket event handlers MUST have cleanup patterns. Violations block code review merge and fail CI pipeline verification.
</enforcement>