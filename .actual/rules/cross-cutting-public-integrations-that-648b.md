# Adopt Async Event-Driven Concurrency with Cache Layer for Public API Integration: Public Integrations That
...
- **R-ASYNC-001** MUST: All public API integrations that involve asynchronous operations MUST implement explicit concurrency control using async/await patterns or promise-based coordination.
...
- **R-ASYNC-006** SHOULD: Apply lodash.throttle to high-frequency broadcast operations (_broadcastSocketData) to prevent network flooding.
- **R-ASYNC-007** SHOULD: Distinguish between volatile (WS_EVENTS.SERVER_VOLATILE) and persistent (WS_EVENTS.SERVER) broadcasts based on data criticality.
...
# Verify async patterns in public API contracts
...
<enforcement>
Clause Code MUST NOT skip or defer verification. All rules in this file are mandatory for code review approval and CI pipeline validation.
</enforcement>